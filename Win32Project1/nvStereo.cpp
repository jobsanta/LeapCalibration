...
// Create / Destroy
StereoParametersD3D11::StereoParametersD3D11() :
	m_StereoParamsMap(0),
	m_StereoParamsMapSRV(0)
{}
...
// Create the graphics objects
void StereoParametersD3D11::createGraphics(ID3D11Device* pd3dDevice)
{
...
void StereoParametersD3D11::destroyGraphics()
{
	if (m_StereoParamsMap)
		m_StereoParamsMap->Release();
	m_StereoParamsMap = 0;
...
// Update the stereo parameters data
// Call this function everytime the stereo Separation or the stereo Convergence changes
void StereoParametersD3D11::updateStereoParamsMap(ID3D11Device*  pd3dDevice, ID3D11DeviceContext* pd3dDeviceContext, float eyeSeparation, float separation, float convergence)
{
	// allocate the Stereo Map if not done yet
	if (!m_StereoParamsMap)
		createGraphics(pd3dDevice);
...
#define     SIH_SWAP_EYES               0x00000001
	...
	pd3dDeviceContext->CopySubresourceRegion(m_StereoParamsMap, 0, 0, 0, 0, sysTexture, 0, &stereoSrcBox);
